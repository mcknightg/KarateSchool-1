    ...
    init:function() {
        ...
        JSONEditor.defaults.resolvers.unshift(function (schema) {
            if (schema.type === "string" && schema.format === "icon") {
                ...
                return "formChooser";
            }else if(schema.type === "string" && schema.format === "imageChooser"){
                return "imageChooser";
            }else if(schema.type === "string" && schema.format === "containerChooser"){
                return "containerChooser";
            }else if(schema.type === "string" && schema.format === "pageChooser"){
                return "pageChooser";
            }
        });
    }
...
JSONEditor.defaults.editors.imageChooser = JSONEditor.defaults.editors.string.extend({
    afterInputReady: function() {
        this._super();
        var self = this;
        var span =  jQuery('<span class="input-group-btn"></span>');
        ...
        span.append(button);
        jQuery(this.input).parent().css('padding-left','15px').css('padding-right','15px').addClass("input-group").append(span);
        jQuery(button).unbind('click').bind('click',function(e){
            e.preventDefault();
            window.fileBrowser.browseFiles(function(path){
...
JSONEditor.defaults.editors.containerChooser = JSONEditor.defaults.editors.string.extend({
    buildList:function(dropDown){
        var self = this;
        jQuery.each(window.lacetInspector.buildContainerDropList(),function(index,column){
...
JSONEditor.defaults.editors.pageChooser = JSONEditor.defaults.editors.string.extend({
    buildList:function(dropDown){
        var self = this;
        window.pages.qry().find(function(data){
            var items = [];
            var sortedItems = [];
            items.push('<li><a href="#" id="None"> None</a></li>');
            jQuery.each(data.rows,function(index,item) {
                sortedItems.push('<li><a href="#" id="' + item._id + '"> ' + item.name + '</a></li>');
            });
            sortedItems.sort();
            jQuery.each(sortedItems,function(index,jitem){
                items.push(jitem);
            });
            jQuery.each(items,function(index,jitem){
                var item = jQuery( jitem);
