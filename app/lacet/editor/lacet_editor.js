(function () {
    ...
    LacetEditor.prototype.addSnippet = function (snippet,insertAbove) {
        this.saveState();
        if(this.selectedcontrol && this.selectedcontrol && this.selectedcontrol.isContainer() && (!this.selectedcontrol.properties.fillPage || this.selectedcontrol.properties.fillPage == 'None') ){
    ...
    LacetEditor.prototype.adjustSideBarHeight = function(){
        var mainContent = $('#main-content');
        $('#east-panel').css("min-height",mainContent.height() +  mainContent[0].scrollHeight);
    };
    ...
    LacetEditor.prototype.saveState = function(){
        var currentPage = this.page.saveObject();
        window.persistence.save(this.page.id,currentPage);
    };
    LacetEditor.prototype.undo = function(){
        var oldPage = window.persistence.undo(this.page.id,this.page);
        if(oldPage){
            this.page = this.page.loadObject(oldPage);
            this.render();
        }
    };
    LacetEditor.prototype.redo = function(){
       var oldPage = window.persistence.redo(this.page.id,this.page);
       if(oldPage){
           this.page = this.page.loadObject(oldPage);
           this.render();
       }
    ...
    LacetEditor.prototype.newView = function () {
        var that = this;
        this.page = new SnippetPage("Untitled");
    ...
    LacetEditor.prototype.initSelectable = function(){
        ...
        jQuery('.selectable').unbind('click').bind('click', function (event) {
            if (!that.editing) {
                ...
                if(id === 'main-content'){
                    that.selectedcontrol = that.page;
                } else{
